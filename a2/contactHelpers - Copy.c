...
#include "contacts.h" //might remove later
...
void ContactManagerSystem(void) {//NOTE: TEST LATER, ASK TEACHER ABOUT CALLING 
	...
	{ "7051112222", "7052223333", "7053334444" } },
 {
	...
	do {
		...
		case 1:
			displayContacts(cDatabase, MAXCONTACT+1);
			pause();
			break;
	...
	do {
		...
		if (strlen(telNum) == 10) {
			...
			for (i = 0; i < strlen(telNum); i++) { 
				...
				if ( (!isdigit(telNum[i]) ) && (!isspace(telNum[i])) ) //If char array spot is not a digit or space then invalid input
				{
...
void displayContacts(const struct Contact contact[], int size) {
	...
	for (i = 0; i < size+1; i++) {
	 ...
	 if ( (  strlen(contact[i].numbers.cell) == 0) ) { //If contact is invalid then we have reached the end of the all VALID contacts
			displayContactFooter(i); //Display footer with contact count
			break; //end for loop
		}
		else {
